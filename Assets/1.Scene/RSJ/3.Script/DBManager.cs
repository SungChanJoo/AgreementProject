...
{
    private MySqlConnection connection; // DB에 연결하는 클래스
    ...
    private string[] weeklyRankColumns;
    private string[] crewColumns;
    private string[] gameColumns;
    private string[] analyticsColumns;
    private string[] lastplaygameColumns;
    private string[] analyticsProfile1Columns; // Level 1 (venezia_chn 포함)
    private string[] analyticsProfile2Columns; // Level 2,3 (venezia_chn 미포함)
    ...
    // LicenseNumber Default, Table 게임명
    private int clientLicenseNumberDefault = 10000;
    ...
    private void SetStringConnection()
    {
        ...
        stringConnection = $"Server={ip};" +
                         ...
                         $"CharSet=utf8;";
        Debug.Log($"[DB] DB connect info : {stringConnection}");
    }
    ...
    private void InitSetting()
    {
        ...
        // Table - Columns
        userinfoColumns = new string[] { "User_LicenseNumber", "User_Charactor", "User_Name", "User_Profile", "User_Birthday", "User_TotalAnswers", "User_TotalTime", "User_Coin" };
        rankColumns = new string[] { "User_LicenseNumber", "User_Charactor", "User_Profile", "User_Name", "TotalTime", "TotalScore" };
        weeklyRankColumns = new string[] { "User_LicenseNumber", "User_Charactor", "User_Profile", "User_Name", "TotalScore", "TotalTime", "ScorePlace", "TimePlace", "HighestScorePlace", "HighestTimePlace" };
        crewColumns = new string[] { "User_LicenseNumber", "User_Charactor", "LastSelectCrew", "루즈비", "블루비", "베르비", "로조비", "블랙비", "똘이", "고릴라", "고슴도치", "고양이", "까마귀", "남생이", "늑대", "다람쥐", "독수리", "돌고래", "라쿤", "물범", "방울뱀", "범고래", "병아리", "북극여우", "쇠족제비", "수탉", "악어", "양", "여우", "오리너구리", "치타", "캥거루", "코뿔소", "쿼카", "팬더", "펭귄", "해달", "호랑이", "흰토끼", "황금독수리" };
        gameColumns = new string[] { "User_LicenseNumber", "User_Charactor", "ReactionRate", "AnswerCount", "AnswerRate", "Playtime", "TotalScore", "StarPoint" };
        analyticsColumns = new string[] { "User_LicenseNumber", "User_Charactor", "ReactionRate", "AnswerRate" };
        lastplaygameColumns = new string[] { "User_LicenseNumber", "User_Charactor", "venezia_kor_level1", "venezia_kor_level2", "venezia_kor_level3", "venezia_eng_level1", "venezia_eng_level2", "venezia_eng_level3", "venezia_chn_level", "calculation_level1", "calculation_level2", "calculation_level3", "gugudan_level1", "gugudan_level2", "gugudan_level3" };
        analyticsProfile1Columns = new string[] { "User_LicenseNumber", "User_Charactor", "Venezia_Kor_PlayCount", "Venezia_Kor_ReactionRate", "Venezia_Kor_AnswerRate", "Venezia_Eng_PlayCount", "Venezia_Eng_ReactionRate", "Venezia_Eng_AnswerRate", "Venezia_Chn_PlayCount", "Venezia_Chn_ReactionRate", "Venezia_Chn_AnswerRate", "Calculation_PlayCount", "Calculation_ReactionRate", "Calculation_AnswerRate", "Gugudan_PlayCount", "Gugudan_ReactionRate", "Gugudan_AnswerRate", "LastPlayGame" };
        analyticsProfile2Columns = new string[] { "User_LicenseNumber", "User_Charactor", "Venezia_Kor_PlayCount", "Venezia_Kor_ReactionRate", "Venezia_Kor_AnswerRate", "Venezia_Eng_PlayCount", "Venezia_Eng_ReactionRate", "Venezia_Eng_AnswerRate", "Calculation_PlayCount", "Calculation_ReactionRate", "Calculation_AnswerRate", "Gugudan_PlayCount", "Gugudan_ReactionRate", "Gugudan_AnswerRate", "LastPlayGame" };
    ...
    public string CreateLicenseNumber()
    {
        ...
        int clientLicenseNumber = clientLicenseNumberDefault + count;
    ...
    public string CreateNewCharactorData(int clientlicensenumber, int clientcharactor, string defaultName = null)
    {
        ...
        selectCharactorCount_Command = $"SELECT `{userinfoColumns[1]}` FROM `{table.list[0]}` WHERE `{userinfoColumns[0]}` = '{clientlicensenumber}';";
        ...
        // user_info table
        // _userinfoColumns = { "User_LicenseNumber", "User_Charactor", "User_Name", "User_Profile", "User_Birthday", "User_TotalAnswers", "User_TotalTime", "User_Coin" };
        insertCharactorData_Command = $"INSERT INTO `{table.list[0]}` (`{userinfoColumns[0]}`, `{userinfoColumns[1]}`) VALUES ({clientlicensenumber},{clientcharactor})";
        ...
        for (int i = 2; i < userinfoColumns.Length; i++) // 0 -> licenseNumber, 1 -> charactor
        {
            updateCharactorData_Command = $"UPDATE `{table.list[0]}` SET `{userinfoColumns[i]}` = @value " +
                                          $"WHERE `{userinfoColumns[0]}` = '{clientlicensenumber}' AND `{userinfoColumns[1]}` = '{clientcharactor}';";
        ...
        Debug.Log("[DB] User_info _table complete!");

        // rank table
        // _rankColumns = { "User_LicenseNumber", "User_Charactor", "User_Profile", "User_Name", "TotalTime", "TotalScore" };
        insertCharactorData_Command = $"INSERT INTO `{table.list[1]}` (`{rankColumns[0]}`, `{rankColumns[1]}`) VALUES ({clientlicensenumber},{clientcharactor})";
        mySqlCommand.CommandText = insertCharactorData_Command;
        mySqlCommand.ExecuteNonQuery();
        int rank_valuepart = 0;

        for (int i = 2; i < rankColumns.Length; i++) // 0->licensenumber, 1->charactor
        {
            updateCharactorData_Command = $"UPDATE `{table.list[1]}` SET `{rankColumns[i]}` = '{rank_valuepart}' " +
                                          $"WHERE `{rankColumns[0]}` = '{clientlicensenumber}' AND `{rankColumns[1]}` = '{clientcharactor}';";
            mySqlCommand.CommandText = updateCharactorData_Command;
            mySqlCommand.ExecuteNonQuery();
        }
        ...
        // crew table
        // _crewColumns { "User_LicenseNumber", "User_Charactor", "FinallySelectedCrew", "루즈비", "블루비", "베르비", "로조비", "블랙비", "똘이", "고릴라", "고슴도치", "고양이", "까마귀", "남생이", "늑대", "다람쥐", "독수리", "돌고래", "라쿤", "물범", "방울뱀", "범고래", "병아리", "북극여우", "쇠족제비", "수탉", "악어", "양", "여우", "오리너구리", "치타", "캥거루", "코뿔소", "쿼카", "팬더", "펭귄", "해달", "호랑이", "흰토끼", "황금독수리" };
        insertCharactorData_Command = $"INSERT INTO `{table.list[2]}` (`{crewColumns[0]}`, `{crewColumns[1]}`) VALUES ({clientlicensenumber},{clientcharactor})";
        mySqlCommand.CommandText = insertCharactorData_Command;
        mySqlCommand.ExecuteNonQuery();
        int crew_valuepart;

        for (int i = 2; i < crewColumns.Length; i++)
        {
            ...
            updateCharactorData_Command = $"UPDATE `{table.list[2]}` SET `{crewColumns[i]}` = '{crew_valuepart}' " +
                                          $"WHERE `{crewColumns[0]}` = '{clientlicensenumber}' AND `{crewColumns[1]}` = '{clientcharactor}';";
            mySqlCommand.CommandText = updateCharactorData_Command;
            mySqlCommand.ExecuteNonQuery();
        }
        ...
        // lastplaygame table
        // _lastplaygameColumns = { "User_LicenseNumber", "User_Charactor", "venezia_kor_level1", "venezia_kor_level2", "venezia_kor_level3", "venezia_eng_level1", "venezia_eng_level2", "venezia_eng_level3", "venezia_chn_level", "calculation_level1", "calculation_level2", "calculation_level3", "gugudan_level1", "vgugudan_level2", "gugudan_level3" };
        // 마지막으로 플레이 한 게임이 무엇인지 저장하기 위한 테이블 / 게임,레벨의 밸류(1~6)가 해당 스텝을 플레이함
        insertCharactorData_Command = $"INSERT INTO `{table.list[3]}` (`{lastplaygameColumns[0]}`, `{lastplaygameColumns[1]}`) VALUES ({clientlicensenumber}, {clientcharactor})";
        mySqlCommand.CommandText = insertCharactorData_Command;
        mySqlCommand.ExecuteNonQuery();
        int lastplaygame_valuepart = 1; // default 값 1 -> 스텝 1을 플레이했음

        for (int i = 2; i < lastplaygameColumns.Length; i++)
        {
            updateCharactorData_Command = $"UPDATE `{table.list[3]}` SET `{lastplaygameColumns[i]}` = '{lastplaygame_valuepart}' " +
                                          $"WHERE `{lastplaygameColumns[0]}` = '{clientlicensenumber}' AND `{lastplaygameColumns[1]}` = '{clientcharactor}';";
            mySqlCommand.CommandText = updateCharactorData_Command;
            mySqlCommand.ExecuteNonQuery();
        }
        ...
        insertCharactorData_Command = $"INSERT INTO `{table.list[4]}` (`{analyticsProfile1Columns[0]}`, `{analyticsProfile1Columns[1]}`) VALUES ({clientlicensenumber}, {clientcharactor});";
        mySqlCommand.CommandText = insertCharactorData_Command;
        mySqlCommand.ExecuteNonQuery();
        int analyticsProfile1_valuepart = 0; // default 값 0

        for (int i = 2; i < analyticsProfile1Columns.Length; i++)
        {
            updateCharactorData_Command = $"UPDATE `{table.list[4]}` SET `{analyticsProfile1Columns[i]}` = {analyticsProfile1_valuepart} " +
                                          $"WHERE `{analyticsProfile1Columns[0]}` = '{clientlicensenumber}' AND `{analyticsProfile1Columns[1]}` = '{clientcharactor}';";
            mySqlCommand.CommandText = updateCharactorData_Command;
            mySqlCommand.ExecuteNonQuery();
        }
        ...
        for (int i = 0; i < 2; i++)
        {
            insertCharactorData_Command = $"INSERT INTO `{table.list[5 + i]}` (`{analyticsProfile2Columns[0]}`, `{analyticsProfile2Columns[1]}`) VALUES ({clientlicensenumber}, {clientcharactor});";
            mySqlCommand.CommandText = insertCharactorData_Command;
            mySqlCommand.ExecuteNonQuery();
            int analyticsProfile2_valuepart = 0; // default 값 0

            for (int j = 2; j < analyticsProfile2Columns.Length; j++)
            {
                updateCharactorData_Command = $"UPDATE `{table.list[5 + i]}` SET `{analyticsProfile2Columns[j]}` = {analyticsProfile2_valuepart} " +
                                              $"WHERE `{analyticsProfile2Columns[0]}` = '{clientlicensenumber}' AND `{analyticsProfile2Columns[1]}` = '{clientcharactor}';";
                mySqlCommand.CommandText = updateCharactorData_Command;
                mySqlCommand.ExecuteNonQuery();
            }
        }
        ...
        //string[] _gameColumns = { "User_LicenseNumber", "User_Charactor", "ReactionRate", "AnswerCount", "AnswerRate", "Playtime", "TotalScore", "StarPoint" };
        ...
        for (int i = 0; i < game_TableList.Count; i++)
        {
            ...
            for (int j = 2; j < gameColumns.Length; j++)
            {
                updateCharactorData_Command = $"UPDATE {game_TableList[i]} SET {gameColumns[j]} = @value  " +
                                              $"WHERE `{gameColumns[0]}` = '{clientlicensenumber}' AND `{gameColumns[1]}` = '{clientcharactor}'";
                mySqlCommand.CommandText = updateCharactorData_Command;
    ...
    public void SaveCharactorName(List<string> filterList)
    {
        ...
        update_Command = $"UPDATE `{table.list[0]}` SET `{userinfoColumns[2]}` = '{dataList[3]}' " +
                         $"WHERE `{userinfoColumns[0]}` = '{Int32.Parse(dataList[1])}' AND `{userinfoColumns[1]}` = '{Int32.Parse(dataList[2])}';";
        update_sqlCmd = new MySqlCommand(update_Command, connection);
        update_sqlCmd.ExecuteNonQuery();

        // rank == table.list[1]
        update_Command = $"UPDATE `{table.list[1]}` SET `{rankColumns[3]}` = '{dataList[3]}' " +
                         $"WHERE `{rankColumns[0]}` = '{Int32.Parse(dataList[1])}' AND `{rankColumns[1]}` = '{Int32.Parse(dataList[2])}';";
        update_sqlCmd = new MySqlCommand(update_Command, connection);
        update_sqlCmd.ExecuteNonQuery();
    ...
    public void SaveCharactorProfile(List<string> filterList)
    {
        ...
        // user_info == table.list[0]
        update_Command = $"UPDATE `{table.list[0]}` SET `{userinfoColumns[3]}` = '{dataList[3]}' " +
                         $"WHERE `{userinfoColumns[0]}` = '{Int32.Parse(dataList[1])}' AND `{userinfoColumns[1]}` = '{Int32.Parse(dataList[2])}';";
        ...
        // rank == table.list[1]
        update_Command = $"UPDATE `{table.list[1]}` SET `{rankColumns[2]}` = '{dataList[3]}' " +
                         $"WHERE `{rankColumns[0]}` = '{Int32.Parse(dataList[1])}' AND `{rankColumns[1]}` = '{Int32.Parse(dataList[2])}';";
        update_sqlCmd = new MySqlCommand(update_Command, connection);
        update_sqlCmd.ExecuteNonQuery();
    ...
    public void SaveCharactorBirthday(List<string> filterList)
    {
        ...
        string update_Command = $"UPDATE `{table.list[0]}` SET `{userinfoColumns[4]}` = '{dataList[3]}' " +
                                $"WHERE `{userinfoColumns[0]}` = '{Int32.Parse(dataList[1])}' AND `{userinfoColumns[1]}` = '{Int32.Parse(dataList[2])}';";
    ...
    public void SaveCharactorData(List<string> filterList)
    {
        ...
        for (int i = 1; i < filterList.Count; i++)
        {
            ...
            else // game table
            {
                update_Command = $"UPDATE `{tempAllocate[0]}` SET ";
                tempAllocate.RemoveAt(0);
                for (int j = 2; j < gameColumns.Length; j++) // 0,1 - license, charactor
                {
                    if (j == 2) update_Command += $"`{gameColumns[j]}` = {float.Parse(tempAllocate[j - 2])}, "; //reactionrate, float
                    else if (j == 3) update_Command += $"`{gameColumns[j]}` = {Int32.Parse(tempAllocate[j - 2])}, "; // answercount, int
                    else if (j == 4) update_Command += $"`{gameColumns[j]}` = {Int32.Parse(tempAllocate[j - 2])}, "; // answerrate, int
                    else if (j == 5) update_Command += $"`{gameColumns[j]}` = {float.Parse(tempAllocate[j - 2])}, "; // playtime, float
                    else if (j == 6) update_Command += $"`{gameColumns[j]}` = {Int32.Parse(tempAllocate[j - 2])}, "; // totalscore, int
                    else if (j == 7) update_Command += $"`{gameColumns[j]}` = {Int32.Parse(tempAllocate[j - 2])} "; // starpoint, int
                }
                update_Command += $"WHERE `{gameColumns[0]}` = {clientLicenseNumber} AND `{gameColumns[1]}` = {clientCharactor};";
            }
    ...
    public void SaveCrewData(List<string> filterList)
    {
        ...
        for (int i = 2; i < crewColumns.Length; i++) // license, charactor 제외
        {
            update_Command += $" `{crewColumns[i]}` = '{Int32.Parse(dataList[i + 1])}',";
        }
        update_Command = update_Command.TrimEnd(',');
        update_Command += $" WHERE `{crewColumns[0]}` = '{Int32.Parse(dataList[1])}' AND `{crewColumns[1]}` = '{Int32.Parse(dataList[2])}'";
    ...
    public void SaveLastPlayData(List<string> filterList)
    {
        ...
        for (int i = 2; i < lastplaygameColumns.Length; i++) // license, charactor 제외
        {
            update_Command += $" `{lastplaygameColumns[i]}` = '{Int32.Parse(dataList[i + 1])}',";
        }
        update_Command = update_Command.TrimEnd(',');
        update_Command += $" WHERE `{lastplaygameColumns[0]}` = '{Int32.Parse(dataList[1])}' AND `{lastplaygameColumns[1]}` = '{Int32.Parse(dataList[2])}'";
    ...
    public void SaveGameResultData(List<string> filterList)
    {
        ...
        // DB gametable column순 : User_Licensenumber/User_Charactor/ReactionRate/AnswerCount/AnswerRate/Playtime/TotalScore/StarPoint
        string selectScoreStarQuery = $"SELECT `{gameColumns[6]}`, `{gameColumns[7]}` FROM `{table_Name}` " +
                                      $"WHERE `{gameColumns[0]}` = '{licenseNumber}' AND `{gameColumns[1]}` = '{charactor}';";
        ...
        {
            updateScoreStartQuery = $"UPDATE `{table_Name}` SET `{gameColumns[6]}` = '{totalScore}' " +
                                    $"WHERE `{gameColumns[0]}` = '{licenseNumber}' AND `{gameColumns[1]}` = '{charactor}';";
        ...
        {
            updateScoreStartQuery = $"UPDATE `{table_Name}` SET `{gameColumns[7]}` = '{totalScore}' " +
                                    $"WHERE `{gameColumns[0]}` = '{licenseNumber}' AND `{gameColumns[1]}` = '{charactor}';";
            mySqlCommand.CommandText = updateScoreStartQuery;
            mySqlCommand.ExecuteNonQuery();
        }
        ...
        // Game Datas Update
        for (int i = 2; i < gameColumns.Length - 2; i++) // _gameColumns = [0]~[5]
        {
            string updateGameDataQuery = $"UPDATE `{table_Name}` SET `{gameColumns[i]}` = @value ";
            ...
            updateGameDataQuery += $"WHERE `{gameColumns[0]}` = '{licenseNumber}' AND `{gameColumns[1]}` = '{charactor}'";
    ...
    private void UpdateUserInfoTableForScoreTime(int licensenumber, int charactor, int answercount, float totaltime)
    {
        ...
        // _userinfoColumns = "User_LicenseNumber", "User_Charactor", "User_Name", "User_Profile", "User_Birthday", "User_TotalAnswers", "User_TotalTime", "User_Coin"
        string selectQuery = $"SELECT `{userinfoColumns[5]}`, `{userinfoColumns[6]}` FROM `{table.list[0]}` " +
        ...
        string updateQuery = $"UPDATE `{table.list[0]}` SET `{userinfoColumns[5]}` = '{dbAnswerCount}', `{userinfoColumns[6]}` = '{dbTotalTime}' " +
                             $"WHERE `{userinfoColumns[0]}` = '{licensenumber}' AND `{userinfoColumns[1]}` = '{charactor}';";
    ...
    private void UpdateRankTable(int licensenumber, int charactor, float time, int score)
    {
        ...
        while (reader.Read())
        {
            if ((reader.GetInt32(rankColumns[0]) == licensenumber) && (reader.GetInt32(rankColumns[1]) == charactor))
            {
                totalTime = reader.GetFloat(rankColumns[4]);
                totalScore = reader.GetInt32(rankColumns[5]);
            }
        }
        reader.Close();
        ...
        string updateRankData_Command = $"UPDATE {rankTable} SET {rankColumns[4]} = {totalTime} WHERE {rankColumns[0]} = {licensenumber} AND {rankColumns[1]} = {charactor}";
        MySqlCommand update_SqlCmd = new MySqlCommand(updateRankData_Command, connection);
        update_SqlCmd.ExecuteNonQuery();
        update_SqlCmd.CommandText = $"UPDATE {rankTable} SET {rankColumns[5]} = {totalScore} WHERE {rankColumns[0]} = {licensenumber} AND {rankColumns[1]} = {charactor}";
        update_SqlCmd.ExecuteNonQuery();
    }
    ...
    public List<string> LoadUserData(int clientlicensenumber)
    {
        ...
        // _presentDB의 user_info테이블에 있는 Charactor컬럼을 Count, clientlicensenumber가 일치하는곳에서
        string select_Command = $"SELECT `{userinfoColumns[1]}` FROM `{table.list[0]}` WHERE `{userinfoColumns[0]}` = '{clientlicensenumber}';";
        mySqlCommand.CommandText = select_Command;
        MySqlDataReader reader = mySqlCommand.ExecuteReader();
        ...
        select_Command = $"SELECT * FROM `{table.list[0]}` WHERE `{userinfoColumns[0]}` = '{clientlicensenumber}';";
        mySqlCommand.CommandText = select_Command;
        reader = mySqlCommand.ExecuteReader();

        while (reader.Read())
        {
            string charactorNumber = reader.GetInt32($"{userinfoColumns[1]}").ToString();
            string charactorName = reader.GetString($"{userinfoColumns[2]}");
            string charactorProfile = reader.GetString($"{userinfoColumns[3]}");
    ...
    public List<string> LoadRankData(int licensenumber, int charactor)
    {
        ...
        if (result == null)
        {
            Debug.Log($"[DB] WeeklyRankTable is not exist on DB, _referenceWeeklyRankTableName : {referenceWeeklyRankTableName}");
        ...
        string clientRankScoreSelectQuery = $"SELECT * FROM `{weeklyRankDB}`.`{referenceWeeklyRankTableName}` " +
                                            $"WHERE `{weeklyRankColumns[0]}` = '{licensenumber}' AND `{weeklyRankColumns[1]}` = '{charactor}';";
    ...
    public List<string> LoadExpenditionCrew(int licensenumber, int charactor)
    {
        ...
        // 조회할 table
        string select_Command = $"SELECT * FROM `{table.list[2]}` WHERE `{crewColumns[0]}` = {licensenumber} AND `{crewColumns[1]}` = {charactor};";
        mySqlCommand.CommandText = select_Command;
        MySqlDataReader reader = mySqlCommand.ExecuteReader();

        while (reader.Read())
        {
            for (int i = 2; i < crewColumns.Length; i++) // licensenumber, charactor 제외
            {
                return_List.Add($"{reader.GetInt32(crewColumns[i])}|");
            }
        }
        reader.Close();
    ...
    public List<string> LoadLastPlayData(int licensenumber, int charactor)
    {
        ...
        string select_Command = $"SELECT * FROM `{table.list[3]}` WHERE `{lastplaygameColumns[0]}` = {licensenumber} AND `{lastplaygameColumns[1]}` = {charactor};";
        mySqlCommand.CommandText = select_Command;
        MySqlDataReader reader = mySqlCommand.ExecuteReader();

        while (reader.Read())
        {
            for (int i = 2; i < lastplaygameColumns.Length; i++) // licensenumber, charactor 제외
            {
                return_List.Add($"{reader.GetInt32(lastplaygameColumns[i])}|");
            }
        }
        reader.Close();
    ...
    public List<string> ChangeCharactor(int licensenumber, int charactor)
    {
        ...
        // 조회할 table -> user_info
        string select_Command = $"SELECT {userinfoColumns[1]} FROM `{table.list[0]}` WHERE `{userinfoColumns[0]}` = {licensenumber} AND `{userinfoColumns[1]}` = {charactor};";
        mySqlCommand.CommandText = select_Command;
        MySqlDataReader reader = mySqlCommand.ExecuteReader();

        while (reader.Read())
        {
            return_List.Add($"{reader.GetInt32(userinfoColumns[1])}|");
        }
        reader.Close();
    ...
    public void CreateDateDB()
    {
        ...
        Debug.Log($"[DB] Created DateDB's Name : {DBName}");
        ...
        // Table생성 명령문
        // 한 테이블당 생성하고 컬럼들 생성
        // _gameColumns = new string[]{ "User_LicenseNumber", "User_Charactor", "ReactionRate", "AnswerCount", "AnswerRate", "Playtime", "TotalScore", "StarPoint" };
        for (int i = 0; i < gameTable.list.Count; i++)
        {
            ...
            for (int j = 0; j < gameColumns.Length; j++)
            {
                ...
                {
                    createTable_Command += $"{gameColumns[j]} float DEFAULT NULL,";
                }
                else // int -> 나머지
                {
                    createTable_Command += $"{gameColumns[j]} int(11) DEFAULT NULL,";
                }
            }
        ...
        for (int i = 0; i < gameTable.list.Count; i++)
        {
            ...
            //reader.Read()는 모든 행을 읽는데, 1행->2행->3행순으로.. 해당 테이블에 있는 모든 데이터를 읽는다
            while (reader.Read())
            {
                // table에 있는 Column들의 Data를 담은 List
                List<string> column_Values = new List<string>();

                for (int k = 0; k < gameColumns.Length; k++)
                {
                    if (k == 2 || k == 5) // float
                    {
                        column_Values.Add(reader.GetFloat(gameColumns[k]).ToString());
                    }
                    else // int
                    {
                        column_Values.Add(reader.GetInt32(gameColumns[k]).ToString());
                    }
        ...
        Debug.Log("[DB] Complete copy _presentDB");
        ...
        for (int i = 0; i < gameTable.list.Count; i++) // Table
        {
            ...
            for (int j = 0; j < valuesInColumnsInTable_Array[i].Count; j++) // Columns
            {
                ...
                string insert_Command = $"INSERT INTO {gameTable.list[i]} (`{gameColumns[0]}`,`{gameColumns[1]}`) " +
                ...
                {
                    string update_Command = $"UPDATE {gameTable.list[i]} SET `{gameColumns[k]}` = @value " +
                                            $"WHERE `{gameColumns[0]}` = '{Int32.Parse(valuesInColumnsInTable_Array[i][j][0])}' AND `{gameColumns[1]}` = '{Int32.Parse(valuesInColumnsInTable_Array[i][j][1])}';";
                    mySqlCommand.CommandText = update_Command;
        ...
        Debug.Log("[DB] Complete paste _presentDB to DateDB only gamedatas");
        ...
        // 그 테이블의 행 개수 valuesInColumnsInTable_Array[i].Count -> Array, 위에서 _presentDB의 데이터를 가져올 때 행의 개수를 알고 있음
        ...
        Debug.Log($"[DB] Check _etcMethodHandler is null? : {(etcMethodHandler == null ? "it's null" : "it's initialized")}");
        ...
        Debug.Log("[DB] Complete _etcMethodHandler.AddAnalyticsColumnValueInDB");
        ...
        for (int i = 0; i < valueList_ColumnArray_TableArray.Length; i++) // 13개 table
        {
            ...
            for (int j = 0; j < analyticsColumns.Length; j++)
            {
                if (j == 2) // float -> ReactionRate
                {
                    createAnalyticsTable_Command += $"{analyticsColumns[j]} float DEFAULT NUll,";
                }
                else
                {
                    createAnalyticsTable_Command += $"{analyticsColumns[j]} int(11) DEFAULT NULL,";
                }
            }
            ...
            for (int j = 0; j < valueList_ColumnArray_TableArray[i].Length; j++) // 각 table안에 존재하는 행의 개수(유저마다 가지고 있는 캐릭터의 합)
            {
                ...
                string insert_Command = $"INSERT INTO `{analyticsTable.list[i]}` ({analyticsColumns[0]}, {analyticsColumns[1]}, {analyticsColumns[2]}, {analyticsColumns[3]}) " +
    ...
    private string SetWeeklyRankTableName(DateTime _dbStandardDay)
    {
        Debug.Log($"[DB] Come in SetWeeklyRankTableName..");

        _dbStandardDay = _dbStandardDay.AddDays(-1);
        string standardDay_yesterday = $"{_dbStandardDay.Year.ToString().Substring(2, 2)}.{_dbStandardDay.Month:00}.{_dbStandardDay.Day:00}";
        _dbStandardDay = _dbStandardDay.AddDays(-6);
        string standardDay_weekago = $"{_dbStandardDay.Year.ToString().Substring(2, 2)}.{_dbStandardDay.Month:00}.{_dbStandardDay.Day:00}";
