...
public class ClientTest : MonoBehaviour
{
    ...
    {
        testPlayer_DB = Client.instance.AppStart_LoadCharactorDataFromDB();
        Debug.Log($"[ClientTest] use Client.instance.AppStart_LoadCharactorDataFromDB(), Current time : {System.DateTime.Now}");
    ...
    public void OnClickCheckLoadedDatas()
    {
        // UserData
    ...
    #region DB로부터 Data들 한 class씩 Load
    public void OnClickLoadUserDataTest()
    {
        testUserData = Client.instance.AppStart_LoadUserDataFromDB();
    }

    public void OnClickLoadPlayer_DBDataTest()
    {
        testPlayer_DB = Client.instance.AppStart_LoadCharactorDataFromDB();
    }

    public void OnClickLoadAnalyticsDataTest()
    {
        testAnalyticsData = Client.instance.AppStart_LoadAnalyticsDataFromDB();
    }
    ...
    public void OnClickLoadRankDataTest()
    {
        ...
        for (int i = 0; i < testRankData.rankdata_score.Length; i++)
        {
            Debug.Log($"testRankData.rankdata_score[i].place : {testRankData.rankdata_score[i].scorePlace}");
            Debug.Log($"testRankData.rankdata_score[i].userProfile : {testRankData.rankdata_score[i].userProfile}");
            Debug.Log($"testRankData.rankdata_score[i].userName : {testRankData.rankdata_score[i].userName}");
            Debug.Log($"testRankData.rankdata_score[i].totalScore : {testRankData.rankdata_score[i].totalScore}");
        }

        for (int i = 0; i < testRankData.rankdata_time.Length; i++)
        {
            Debug.Log($"testRankData.rankdata_time[i].place : {testRankData.rankdata_time[i].timePlace}");
            Debug.Log($"testRankData.rankdata_time[i].userProfile : {testRankData.rankdata_time[i].userProfile}");
            Debug.Log($"testRankData.rankdata_time[i].userName : {testRankData.rankdata_time[i].userName}");
            Debug.Log($"testRankData.rankdata_time[i].totalScore : {testRankData.rankdata_time[i].totalTime}");
        }
    }

    public void OnClickLoadExpendtionCrewDataTest()
    {
        testExpenditionCrew = Client.instance.AppStart_LoadExpenditionCrewFromDB();
    }

    public void OnClickLoadLastPlayDataTest()
    {
        testLastPlayData = Client.instance.AppStart_LoadLastPlayFromDB();
    }

    public void OnClickLoadAnalyticsProfileDataTest()
    {
        testAnalyticsProfileData = Client.instance.AppStart_LoadAnalyticsProfileDataFromDB();
    }
    ...
    #region Save example data to DB
    public void OnClickSaveCharactorDataTest()
    {
        ...
        Client.instance.AppExit_SaveCharactorDataToDB(player_DB);
    }

    public void OnClickSaveExpenditionCrewDataTest()
    {
        int testSelectedCrew = 3; //선택한 대원
        List<bool> testOwnedCrew = new List<bool>(); //보유한 대원 리스트

        for(int i = 0; i < 37; i ++)
        {
            testOwnedCrew.Add(true);
        }

        ExpenditionCrew exampleExpendtionCrewData = new ExpenditionCrew(testSelectedCrew, testOwnedCrew);

        Client.instance.AppExit_SaveExpenditionCrewDataToDB(exampleExpendtionCrewData);
    }
    ...
    public void OnClickSaveLastPlayGameTest()
    {
        ...
        LastPlayData exampleLastPlayData = new LastPlayData();

        // 게임종류 5, level 3 / venezia_chn은 level 1만
        exampleLastPlayData.Step.Add((Game_Type.A, 1), 2);
        exampleLastPlayData.Step.Add((Game_Type.A, 2), 2);
        exampleLastPlayData.Step.Add((Game_Type.A, 3), 2);
        exampleLastPlayData.Step.Add((Game_Type.B, 1), 2);
        exampleLastPlayData.Step.Add((Game_Type.B, 2), 2);
        exampleLastPlayData.Step.Add((Game_Type.B, 3), 2);
        exampleLastPlayData.Step.Add((Game_Type.C, 1), 2);
        exampleLastPlayData.Step.Add((Game_Type.D, 1), 2);
        exampleLastPlayData.Step.Add((Game_Type.D, 2), 2);
        exampleLastPlayData.Step.Add((Game_Type.D, 3), 2);
        exampleLastPlayData.Step.Add((Game_Type.E, 1), 2);
        exampleLastPlayData.Step.Add((Game_Type.E, 2), 2);
        exampleLastPlayData.Step.Add((Game_Type.E, 3), 2);

        Client.instance.AppExit_SaveLastPlayDataToDB(exampleLastPlayData);
    }

    public void OnClickSaveGameDataTest()
    {
        // 게임,레벨,스텝
        // 게임데이터 - 반응속도(reactionRate), 정답수(answercount), 정답률(answerRate), 플레이시간(playTime), 점수(score), 별개수(starPoint)
        Game_Type game_Type = Game_Type.A;
        int level = 1;
        int step = 2;
